...
Criação do Banco: O nome do banco de dados utilizado é RedeSocial. Você deve criá-lo no seu PostgreSQL.

Scripts Iniciais: A API requer que as tabelas sejam criadas antecipadamente. Execute o conteúdo do arquivo data.sql (que contém o CREATE TABLE para usuario, amizade, postagem, etc.) na query do seu DB RedeSocial.

Configuração de Credenciais: Configure suas credenciais no arquivo src/main/resources/application.yml.

Exemplo de configuração: url: jdbc:postgresql://localhost:5432/RedeSocial, username: SEU_USUARIO, password: SUA_SENHA.
Instalação e Execução
Para iniciar o servidor da API:

Instalação de Dependências: Acesse a pasta do projeto da API e utilize o Maven: mvn clean install.

Execução da API: Inicie o Spring Boot: mvn spring-boot:run.
